        ...
        // Vote webhook endpoint for top.gg
            ...
            try {
                ...
                if (type === 'upvote') {
                    ...
                    if (recentServerId) {
                        // Give reward to the specific server (35 translations per Top.gg vote)
                        const result = await monetizationService.handleVoteReward(userId, recentServerId, 35);
                        
                        if (result.success) {
                            console.log(`✅ Vote reward (35 translations) processed for user ${userId} in server ${recentServerId}`);
        ...
        // Test vote webhook endpoint (for debugging)
            ...
            try {
                ...
                if (userId && serverId) {
                    ...
                    const result = await monetizationService.handleVoteReward(userId, serverId, 35);
                    
                    if (result.success) {
                        console.log(`✅ Test vote reward (35 translations) processed for user ${userId} in server ${serverId}`);
